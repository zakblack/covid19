...
export default async (req: NowRequest, response: NowResponse) => {
  try {
    ...
    response.json({
      confirmed: {
        value: confirmed,
        detail: `https://covid19.zakaria.tech/api/countries/${country}/confirmed`
      },
      recovered: {
        value: recovered,
        detail: `https://covid19.zakaria.tech/api/countries/${country}/recovered`
      },
      deaths: {
        value: deaths,
        detail: `https://covid19.zakaria.tech/api/countries/${country}/deaths`
      },
      lastUpdate
    });
  } catch (error) {
